...
async def run(accounts, target_uid, q):
...
def benang(accounts, target_uid, q):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    try:
        loop.run_until_complete(run(accounts, target_uid, q))
    except Exception as e:
...
def stream():
    ...
    t = threading.Thread(target=benang, args=(accounts, target_uid, q), daemon=True)
    t.start()
